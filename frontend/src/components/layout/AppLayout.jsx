...
function AppLayoutInner() {
  ...
  const { isSidebarHidden, isHeaderHidden, isContentPaddingHidden } = useLayout();
  ...
  return (
    <div className="min-h-dvh bg-transparent">
      ...
      <div className={`min-h-dvh min-w-0 ${isSidebarHidden ? "" : "md:ml-[17rem]"}`}>
        ...
        <main className={`min-h-dvh ${isHeaderHidden ? "" : "pt-16"}`}>
          <div className={isContentPaddingHidden ? "" : "px-3 py-4 sm:px-5 sm:py-5 md:px-6 md:py-6 lg:px-8 lg:py-8"}>
            <Outlet />
          </div>
        </main>
      </div>
